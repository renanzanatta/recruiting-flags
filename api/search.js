export default async function handler(req, res) {
  try {
    ...
    // emojis are accepted but NOT used as a required search term here
    ...
    // Name: exact phrase (helps precision)
    if (name) {
      const quotedName = `"${name.replaceAll('"', "")}"`;
      parts.push(quotedName);
      // NOTE: we intentionally removed intitle: because it can kill results in CSE
    }
    ...
    return res.status(200).json({
      ...
      note:
        emojis.length
          ? "Emojis serão filtrados somente no TÍTULO (aproximação de 'emoji no nome')."
          : null
    });
  } catch (e) {
